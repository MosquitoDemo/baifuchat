...
class EditEmailCell: UITableViewCell {
    typealias UITextFieldBlock = ((UITextField)->Void)
    var user:User?{
        didSet{
            
            self.detailTextField.text = user?.emails.first?.email
        }
    }
    ...
    var emailChangedBlock:UITextFieldBlock?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.detailTextField.addTarget(self, action: #selector(EditEmailCell.editEmailChanged(_:)), for: .editingChanged)
    ...
    @objc func editEmailChanged(_ textField:UITextField){
        self.emailChangedBlock?(textField)
    }

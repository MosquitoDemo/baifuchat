...
public class YLDatePicker: UIView {
    ...
    fileprivate var minLimitDate = Date.init(timeIntervalSince1970: TimeInterval(180*24*3600)) // 默认最小时间
    fileprivate var maxLimitDate = Date.init(timeIntervalSince1970: TimeInterval(0)) // 默认最大时间
...
extension YLDatePicker {
    ...
    func getYears() -> Array<String> {
        var years = [String]()
        for year in maxLimitDate.getComponent(component: .year)...minLimitDate.getComponent(component: .year) {
